...
def estilizar_tabela(df, destaques_linha=None, col_total=None):
    """Aplica cores e formatação executiva ao DataFrame para exibição via st.dataframe.

    destaques_linha: dict {idx_linha: cor_hex} — fundo customizado por linha
    col_total: nome da coluna a destacar com fundo (azul claro)
    """
    cor_header = "#0B2D5B"
    cor_alterna = "#F4F7FB"
    cor_destaque = "#D6E4F0"
    ...
    def aplicar(row):
        styles = [""] * len(row)
        idx = row.name
        if destaques_linha and idx in destaques_linha:
            color = destaques_linha[idx]
            styles = [f"background-color: {color}; font-weight: bold;"] * len(row)
        elif idx % 2 == 1:
            styles = [f"background-color: {cor_alterna};"] * len(row)
        return styles
    ...
    styler = styler.set_table_styles([
        {"selector": "thead th",
         "props": [("background-color", cor_header), ("color", "white"),
                   ...
                   ("padding", "8px"), ("border", "1px solid #073166")]},
        {"selector": "tbody td",
         "props": [("padding", "6px 10px"), ("border-bottom", "1px solid #e2e8f0")]},
    ])
    return styler
...
with tabs[0]:
    ...
    # Destaca a linha do mês de referência em verde claro
    idx_ref_local = meses_sel.index(mes_ref) if mes_ref in meses_sel else None
    destaques = {idx_ref_local: "#D4EDDA"} if idx_ref_local is not None else {}
    st.dataframe(estilizar_tabela(margens_df, destaques_linha=destaques),
...
with tabs[1]:
    ...
    acum_df = pd.DataFrame(acum_data)
    # Verde nas linhas de totais importantes (Margem 1, Resultado Bruto, EBITDA, Resultado Líquido)
    destaques = {2: "#A8D5A2", 3: "#A8D5A2", 4: "#A8D5A2", 5: "#A8D5A2"}
    st.dataframe(estilizar_tabela(acum_df, destaques_linha=destaques),
                 use_container_width=True, hide_index=True)
...
with tabs[2]:
    ...
    # Destaca linha TOTAL em azul e a coluna do mês de referência
    destaques = {3: "#B4CFE8"}
    st.dataframe(estilizar_tabela(despesas_df, destaques_linha=destaques, col_total=mes_ref),
                 use_container_width=True, hide_index=True)
...
with tabs[3]:
    ...
    # Cor por grupo
    cores_grupo = {
        "AC": "#DDE9F5",  "ANC": "#B4CFE8",
        "PC": "#FDEBD0",  "PNC": "#F8C471",
        "PL": "#A8D5A2",
    }
    destaques = {i: cores_grupo[bal_show.iloc[i]["Grupo"]]
                 for i in range(len(bal_show)) if bal_show.iloc[i]["Grupo"] in cores_grupo}
    st.dataframe(estilizar_tabela(bal_show, destaques_linha=destaques),
                 use_container_width=True, hide_index=True)
...
with tabs[5]:
    ...
    # Destaca Ciclo Operacional (azul) e Ciclo Financeiro (verde)
    destaques = {3: "#B4CFE8", 4: "#A8D5A2"}
    st.dataframe(estilizar_tabela(ciclo_df, destaques_linha=destaques, col_total=mes_ref),
                 use_container_width=True, hide_index=True)
    ...
    if idx_ant is not None:
        ...
        st.markdown(
            ...
            f"<b>{cf[idx_ref]:.1f} dias</b> "
            f"({'+' if var_cf > 0 else ''}{var_cf:.1f} dias vs. {mes_ant}). "
            f"Quanto menor o ciclo, menor a necessidade de capital de giro.</div>",
            unsafe_allow_html=True,
        )

# ========== ABA 7: DRE COMPLETA (Hierárquica e Colorida) ==========
with tabs[6]:
    st.subheader("DRE Completa — Período Selecionado")
    st.caption("Clique nas linhas com seta ► para expandir os detalhes. Clique novamente para recolher.")
    ...
    ESTILOS = {
        "TOPLINE":   {"cor": COR_HEADER,  "bold": True},
        "DEDUC":     {"cor": COR_HEADER,  "bold": True},
        "LIQUIDA":   {"cor": COR_TOTAL,   "bold": True},
        "OUTRAS":    {"cor": COR_SUBHEAD, "bold": True},
        "CMV":       {"cor": COR_HEADER,  "bold": True},
        "MG1":       {"cor": COR_TOTAL,   "bold": True},
        "OUTROS":    {"cor": COR_HEADER,  "bold": True},
        "RES_BRUTO": {"cor": COR_TOTAL,   "bold": True},
        "COMERCIAL": {"cor": COR_HEADER,  "bold": True},
        "LOGISTICA": {"cor": COR_HEADER,  "bold": True},
        "ADM":       {"cor": COR_HEADER,  "bold": True},
        "EBITDA":    {"cor": COR_TOTAL,   "bold": True},
        "REC_FIN":   {"cor": COR_HEADER,  "bold": True},
        "DESP_FIN":  {"cor": COR_HEADER,  "bold": True},
        "LUCRO":     {"cor": COR_TOTAL,   "bold": True},
        "DET":       {"cor": COR_DET,     "bold": False},
    }

    rows = dre_df.to_dict("records")
    parent_counter = 0
    current_parent_id = None

    html = """
    <style>
        .dre-wrap { font-family: 'Segoe UI', sans-serif; font-size: 13px; }
        .dre-table { width: 100%; border-collapse: collapse; }
        .dre-table th { background: #0B2D5B; color: white; padding: 10px 12px;
                        text-align: left; font-weight: 600; }
        .dre-table th.right { text-align: right; }
        .dre-table td { padding: 7px 12px; border-bottom: 1px solid #e2e8f0;
                        white-space: nowrap; }
        .dre-table td.num { text-align: right; font-variant-numeric: tabular-nums; }
        .dre-table tr.parent { cursor: pointer; user-select: none; }
        .dre-table tr.parent:hover { filter: brightness(0.95); }
        .dre-table tr.detail { display: none; }
        .dre-table tr.detail.show { display: table-row; }
        .toggle { display: inline-block; width: 18px; color: #0B2D5B; font-weight: bold; }
        .toggle.open { color: #166534; }
        .indent { padding-left: 36px !important; color: #4a5568; font-style: italic; }
        .num.negative { color: #b91c1c; }
        .controls { margin: 0 0 12px 0; }
        .controls button {
            background: #0B2D5B; color: white; border: none; padding: 7px 16px;
            border-radius: 4px; cursor: pointer; margin-right: 6px; font-size: 12px;
            font-weight: 600;
        }
        .controls button:hover { background: #1a4480; }
    </style>
    <div class="dre-wrap">
        <div class="controls">
            <button onclick="dreExpandAll()">▼ Expandir tudo</button>
            <button onclick="dreCollapseAll()">▶ Recolher tudo</button>
        </div>
        <table class="dre-table"><thead><tr><th>Conta</th>
    """
    for m in meses_sel:
        html += f'<th class="right">{m}</th>'
    html += "</tr></thead><tbody>"

    def _fmt_dre(v, indent=False):
        if pd.isna(v):
            cls = "num indent" if indent else "num"
            return f'<td class="{cls}">-</td>'
        sinal = "-" if v < 0 else ""
        s = f"{abs(v):,.2f}".replace(",", "_").replace(".", ",").replace("_", ".")
        neg = " negative" if v < 0 else ""
        cls = f"num indent{neg}" if indent else f"num{neg}"
        return f'<td class="{cls}">{sinal}{s}</td>'

    for i, row in enumerate(rows):
        bloco = row["Bloco"]
        conta = row["Conta"] if pd.notna(row["Conta"]) else ""
        estilo = ESTILOS.get(bloco, ESTILOS["DET"])

        if bloco != "DET":
            tem_filhos = (i + 1 < len(rows)) and rows[i + 1]["Bloco"] == "DET"
            parent_counter += 1
            current_parent_id = f"g{parent_counter}"
            bold_css = "font-weight: 700;" if estilo["bold"] else ""
            bg = estilo["cor"]

            if tem_filhos:
                row_class = "parent"
                onclick = f"onclick=\"dreToggle('{current_parent_id}', this)\""
                toggle_html = '<span class="toggle">▶</span> '
            else:
                row_class = ""
                onclick = ""
                toggle_html = '<span class="toggle">&nbsp;</span> '

            html += f'<tr class="{row_class}" style="background:{bg};{bold_css}" {onclick}>'
            html += f'<td>{toggle_html}{conta}</td>'
            for m in meses_sel:
                html += _fmt_dre(row.get(m))
            html += "</tr>"
        else:
            html += f'<tr class="detail" data-parent="{current_parent_id}">'
            html += f'<td class="indent">{conta}</td>'
            for m in meses_sel:
                html += _fmt_dre(row.get(m), indent=True)
            html += "</tr>"
    ...
    <script>
        function dreToggle(id, headerRow) {
            const rows = document.querySelectorAll('tr.detail[data-parent="'+id+'"]');
            const toggle = headerRow.querySelector('.toggle');
            const isOpen = toggle.classList.contains('open');
            rows.forEach(r => { if (isOpen) r.classList.remove('show'); else r.classList.add('show'); });
            toggle.classList.toggle('open');
            toggle.textContent = isOpen ? '▶' : '▼';
        }
        function dreExpandAll() {
            document.querySelectorAll('tr.detail').forEach(r => r.classList.add('show'));
            document.querySelectorAll('.toggle').forEach(t => {
                if (t.textContent.trim() === '▶') { t.textContent = '▼'; t.classList.add('open'); }
            });
        }
        function dreCollapseAll() {
            document.querySelectorAll('tr.detail').forEach(r => r.classList.remove('show'));
            document.querySelectorAll('.toggle').forEach(t => {
                if (t.textContent.trim() === '▼') { t.textContent = '▶'; t.classList.remove('open'); }
            });
        }
    </script>
    """
